...
function drag(e) {
  ...
  if (newHeight >= minHeight - 20 && newHeight <= maxHeight + 20) {
    ...
    // Update border radius for bouncing effect if below 50%
    if (newHeight < minHeight) {
      container.style.borderTopLeftRadius = '8px'; // Change to 8px
      container.style.borderTopRightRadius = '8px'; // Change to 8px
    } else {
      container.style.borderTopLeftRadius = '16px'; // Reset to 16px
      container.style.borderTopRightRadius = '16px'; // Reset to 16px
    }
  }
...
function endDrag() {
  ...
  const inertiaInterval = setInterval(() => {
    ...
    if (targetHeight < minHeight) { // Check if below 50%
      container.style.borderTopLeftRadius = '8px'; // Set to 8px when bouncing
      container.style.borderTopRightRadius = '8px'; // Set to 8px when bouncing
    } else {
      container.style.borderTopLeftRadius = '16px'; // Reset to 16px if above 50%
      container.style.borderTopRightRadius = '16px'; // Reset to 16px if above 50%
    }
