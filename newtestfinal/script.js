...
// Handle click and hold on the touch area
touchArea.addEventListener('mousedown', () => {
    gsap.to(dragHandle, {
      width: '90px',
      height: '3px',
      duration: 0.25, // Smooth, quicker animation on hold
      ease: "power2.inOut"
    });
  
    // Separate background color change for smooth transition
    gsap.to(dragHandle, {
      backgroundColor: '#D0D1D5',
      duration: 0.15, // Slightly faster for color change
      ease: "cubic-bezier(0.25, 0.1, 0.25, 1)" // Smooth Apple-like transition
    });
  });
  ...
  touchArea.addEventListener('mouseup', () => {
    gsap.to(dragHandle, {
      width: '100px',
      height: '5px',
      duration: 0.6, // Longer duration for a slower return
      ease: "elastic.out(0.4, 0.2)" // Softer and slower bounce
    });
  
    // Separate background color change to prevent overbounce
    gsap.to(dragHandle, {
      backgroundColor: '#3D3D42',
      duration: 0.3, // Smooth return for color
      ease: "cubic-bezier(0.25, 0.1, 0.25, 1)" // Smooth Apple-like transition
    });
  });
  ...
  // Handle hover over the touch area
