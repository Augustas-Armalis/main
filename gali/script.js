...
function startDrag(e) {
  ...
  // Remove vibration effect on press
...
function onDrag(e) {
  ...
  const newFlexBasis = (constrainedHeight / window.innerHeight) * 100;

  // Update the styles only if the new height is valid
  if (constrainedHeight >= minHeight - 20 && constrainedHeight <= maxHeight + 20) {
...
function endDrag() {
  ...
  if (formFlexBasis >= 87) {
    targetHeight = 100;
    formContainer.classList.add('full-open');
    // Remove vibration effect on release
    // if (navigator.vibrate) {
