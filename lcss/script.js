...
let startY = 0;
let initialStartHeight = 0; // Fixed height when drag starts
let lastY = 0;
let lastTimestamp = 0;
let velocity = 0;
...
const THRESHOLD = 5; // Threshold to prevent small unintended movements
...
function startDrag(e) {
  ...
  // Lock in initialStartHeight only once during the first drag
  initialStartHeight = formContainer.getBoundingClientRect().height;

  console.log("StartY:", startY, "Initial StartHeight:", initialStartHeight);
...
function onDrag(e) {
  ...
  if (Math.abs(dy) > THRESHOLD) {
    // Maintain a consistent height during drag
    const newHeight = initialStartHeight - dy; // Use the locked height
    ...
    if (newHeight >= minHeight - 20 && newHeight <= maxHeight + 20) {
      ...
      window.requestAnimationFrame(() => {
        ...
        console.log(`Form Flex Basis: ${getComputedStyle(formContainer).flexBasis}, Height: ${formContainer.getBoundingClientRect().height}`);

        // Add/remove classes based on flexBasis
...
// Touch event listeners restored
