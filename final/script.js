...
function startDrag(e) {
  ...
  // Get the initial height and the starting position of the drag
  startY = touchY;
...
function onDrag(e) {
  ...
  const dy = startY - currentY; // Difference from the initial touch position
  const newHeight = startHeight + dy; // Adjusting the height based on the change

  const minHeight = window.innerHeight * 0.5;
  const maxHeight = window.innerHeight;

  // Update the height only if it's within the min and max bounds
  if (newHeight >= minHeight && newHeight <= maxHeight) {
    ...
    window.requestAnimationFrame(() => {
      ...
      // Add or remove classes based on the new flex basis
...
function endDrag() {
  ...
  // Determine the target height based on the final flex basis
  if (formFlexBasis >= 87) {
    targetHeight = 100;
    formContainer.classList.add('full-open');
    if (navigator.vibrate) {
    ...
    targetHeight = Math.max(50, formFlexBasis - (velocity * bounceFactor * 5)); // Ensure targetHeight doesn't go below 50%
  }

  // Ensure targetHeight does not exceed boundaries
